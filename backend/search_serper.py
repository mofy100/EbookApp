...
使用例:
  ...
  python -m backend.search_serper --ids 1,2,3
  python -m backend.search_serper --limit 10 --offset 0
  python -m backend.search_serper --limit 20 --force
"""
...
def gather_web_results(title: str, author: str, api_key: str) -> list[dict]:
    """
    Serper で作品情報を検索し、各ページ本文を収集して返す。
...
def process_book(
    ...
    try:
        web_results = gather_web_results(title, author, serper_api_key)
    except Exception as e:
        print(f"    エラー: {e}")
        return False
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Serper Search で作品情報を収集して保存")
