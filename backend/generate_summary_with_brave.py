#!/usr/bin/env python3
"""
Brave Search API + Claude Haiku で作品 summary.json を生成するスクリプト。

使用例:
  python -m backend.generate_summary_with_brave --id 1000
  python -m backend.generate_summary_with_brave --ids 1,2,3
  python -m backend.generate_summary_with_brave --limit 10 --offset 0
  python -m backend.generate_summary_with_brave --limit 20 --force
"""
...
def brave_search(query: str, api_key: str, count: int = 5) -> list[dict]:
    """Brave Search API で検索し、結果のリストを返す。"""
    headers = {
...
def gather_web_context(title: str, author: str, api_key: str) -> tuple[str, list[str]]:
    """
    Brave Search で作品情報を検索し、ページ本文を収集する。
...
def process_book(
    ...
    try:
        web_context, source_urls = gather_web_context(title, author, brave_api_key)
        if not web_context:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Brave Search + Claude Haiku でサマリーを生成")
